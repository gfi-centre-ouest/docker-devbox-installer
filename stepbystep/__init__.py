...
class UndefinedStepException(Exception):
    """
...
class StepFactory(ABC):
    ...
    def build_step(self, model: StepModel, workflow: 'Workflow', context: 'WorkflowRunContext') -> Optional[Step]:
        ...
        :param context: current workflow run context
...
class Workflow:
    ...
    def run(self):
        ...
        for step_model in self.model.steps:
            ...
            if step is None:
                # In case of step model passed to the Workflow but no step generated by factory
                raise UndefinedStepException(f"The step_model named {step_model.name} is not handled by your factory.")
